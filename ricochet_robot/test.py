...
class RicochetArena:
    ...
    def load_map(self, group_id=None):
        ...
        if os.path.exists(filename):
            try:
                ...
                self.log_msg(f"Đã tải Map: {os.path.basename(filename)} ({self.grid_size}x{self.grid_size})",
                             (0, 255, 255))
            except Exception:
                self.grid_size = 16
    ...
    def setup_csp_map(self):
        self.grid_size = 5  # Map 5x5 cho thuật toán CSP chạy mượt hơn
        self.board_size = min(HEIGHT - 100, WIDTH - 680)
        self.cell_size = self.board_size // self.grid_size
        self.walls.clear()
        ...
        # Chọn Target
        self.target_pos = (random.randint(2, self.grid_size - 3), random.randint(2, self.grid_size - 3))
        
        # Chọn Start sao cho nằm trên cùng hàng hoặc cột với đích (Và cách >= 2 ô)
        start_choices = []
        for x in range(self.grid_size):
            if abs(x - self.target_pos[0]) > 1: start_choices.append((x, self.target_pos[1]))
        for y in range(self.grid_size):
            if abs(y - self.target_pos[1]) > 1: start_choices.append((self.target_pos[0], y))
        
        st = random.choice(start_choices)
        self.target_pos_2 = None
        for r in self.robots:
            r.start_pos = list(st)
            
        for aux in self.aux_robots:
            aux.start_pos = list(self.get_random_valid_pos({self.target_pos, st}))
    ...
    def run_partial_observable(self, start, obs):
        ...
        for step in range(30):
            ...
            self.log_msg(f"   [Cập nhật Niềm tin] Loại bỏ giả thuyết sai. Còn: {len(belief_state)} vị trí.",
                         (255, 255, 100))
    ...
    def get_csp_domain(self, start):
        domain = []
        for x in range(self.grid_size):
            for y in range(self.grid_size):
                if (x, y) != self.target_pos and (x, y) != start:
                    domain.append((x, y))
        return domain

    def calculate_csp_conflicts(self, R_positions, start):
        c = 0
        if len(set(R_positions)) < len(R_positions): c += 20
        for r in R_positions:
            if r == self.target_pos or r == start: c += 20
            
        obstacles = set(R_positions)
        target = self.target_pos
        dx = target[0] - start[0]
        dy = target[1] - start[1]
        
        if dx == 0 and dy == 0: return 9999
        
        dir_t = (1 if dx > 0 else -1 if dx < 0 else 0, 1 if dy > 0 else -1 if dy < 0 else 0)
        dest = self.get_slide_dest(start, dir_t, obstacles)
        
        # Ràng buộc 1: Phải trượt tới đúng đích
        if dest != target:
            c += abs(dest[0] - target[0]) + abs(dest[1] - target[1])
            
        # Ràng buộc 2: 3 hướng còn lại phải bị chặn
        for d in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            if d == dir_t: continue
            if self.get_slide_dest(start, d, obstacles) != start:
                c += 5
                
        return c

    def run_backtracking(self, start, obs):
        self.log_msg("--- BACKTRACKING (Tìm tọa độ 3 Trợ thủ) ---", (180, 100, 255))
        domain = self.get_csp_domain(start)
        
        # Thử ưu tiên các ô xung quanh start và target để giải nhanh
        priority_domain = []
        for x, y in domain:
            dist = min(abs(x - start[0]) + abs(y - start[1]), abs(x - self.target_pos[0]) + abs(y - self.target_pos[1]))
            priority_domain.append((dist, (x, y)))
        priority_domain.sort(key=lambda item: item[0])
        sorted_domain = [item[1] for item in priority_domain]
        
        def backtrack(assignment):
            if self.current_group_id == 5 and len(assignment) > 0:
                for i, pos in enumerate(assignment):
                    self.aux_robots[i].visual_pos = [pos[0] * self.cell_size, pos[1] * self.cell_size]
                self.draw_simulation()
                pygame.display.flip()
                pygame.time.delay(50)  # Thêm delay cứng 50ms để user dễ xem
        ...
        if res != "failure":
            for i, aux in enumerate(self.aux_robots):
                aux.logic_pos = list(res[i])
                aux.start_pos = list(res[i])
            return [self.target_pos]
        return []

    def run_ac3(self, start, obs):
        self.log_msg("--- AC-3 (Lọc miền giá trị rồi Backtracking) ---", (147, 112, 219))
        domain = self.get_csp_domain(start)
        
        # AC-3 lọc miền giá trị (cắt tỉa)
        # Nếu trợ thủ không nằm quanh start hoặc target thì không thể chặn hay dừng robot được!
        filtered_domain = []
        for x, y in domain:
            is_useful = False
            for dx, dy in [(0,1), (0,-1), (1,0), (-1,0)]:
                if (x, y) == (start[0]+dx, start[1]+dy): is_useful = True
                if (x, y) == (self.target_pos[0]+dx, self.target_pos[1]+dy): is_useful = True
            if is_useful: filtered_domain.append((x, y))
    ...
    def run_min_conflicts(self, start, obs):
        self.log_msg("--- MIN-CONFLICTS (Tối thiểu hóa xung đột 3 Trợ thủ) ---", (255, 140, 0))
        domain = self.get_csp_domain(start)
        ...
        for step in range(200):
            ...
            if self.current_group_id == 5:
                for i, aux in enumerate(self.aux_robots):
                    aux.visual_pos = [current[i][0] * self.cell_size, current[i][1] * self.cell_size]
                self.draw_simulation()
                pygame.display.flip()
                pygame.time.delay(50)
            ...
            if conflicts == 0:
                for i, aux in enumerate(self.aux_robots):
                    aux.logic_pos = list(current[i])
                    aux.start_pos = list(current[i])
                return [self.target_pos]
                
            var = random.randint(0, 2)
            min_c = float('inf')
            best_vals = []
            sample_domain = random.sample(domain, min(20, len(domain)))
            for v in sample_domain:
                temp = list(current)
                temp[var] = v
    ...
    def get_best_adv_move(self, p_pos, e_pos, depth, is_max, alpha, beta, algo):
        ...
        if is_max:  # NÚT MAX (Lượt của Ta)
            ...
            if algo == "expectimax":  # Nút Chance (Môi trường ngẫu nhiên)
                avg_val = sum(self.get_best_adv_move(p_pos, nxt_e, depth - 1, True, alpha, beta, algo)[0] for nxt_e in
    ...
    def run_adversarial(self, start, obs, algo):
        ...
        for step in range(1, 15):
            # 1. Lượt MAX (Ta)
            _, nxt_p = self.get_best_adv_move(p_curr, e_curr, depth=5, is_max=True, alpha=float('-inf'),
                                              beta=float('inf'), algo=algo)
            ...
            # 2. Lượt MIN (Địch)
            _, nxt_e = self.get_best_adv_move(p_curr, e_curr, depth=4, is_max=False, alpha=float('-inf'),
                                              beta=float('inf'), algo=algo)
    ...
    def draw_simulation(self):
        ...
        if self.edit_mode > 0 and OFFSET_X <= mx <= OFFSET_X + actual_board_size and OFFSET_Y <= my <= OFFSET_Y + actual_board_size:
            ...
            highlight_rect = pygame.Rect(OFFSET_X + hc * self.cell_size, OFFSET_Y + hr * self.cell_size, self.cell_size,
                                         self.cell_size)
            highlight_color = (255, 50, 50, 100) if self.edit_mode == 1 else (
                (50, 255, 50, 100) if self.edit_mode == 2 else (255, 255, 50, 100))
        ...
        if r_active:
            rx, ry = int(OFFSET_X + r_active.visual_pos[0] + self.cell_size // 2), int(
                OFFSET_Y + r_active.visual_pos[1] + self.cell_size // 2)
        ...
        if self.current_group_id == 6:
            ex, ey = int(OFFSET_X + self.enemy.visual_pos[0] + self.cell_size // 2), int(
                OFFSET_Y + self.enemy.visual_pos[1] + self.cell_size // 2)
        ...
        # Vẽ 3 Trợ thủ nếu đang ở nhóm 5
        if self.current_group_id == 5:
            for aux in getattr(self, 'aux_robots', []):
                ax, ay = int(OFFSET_X + aux.visual_pos[0] + self.cell_size // 2), int(
                    OFFSET_Y + aux.visual_pos[1] + self.cell_size // 2)
                self.draw_3d_robot(self.screen, ax, ay, aux.color)
                # Dấu thập chỉ thị trợ thủ
    ...
    def update(self):
        ...
        if r and not is_sliding:
            ...
            if getattr(self, 'step_mode', False) and getattr(self, 'step_queue', []):
                ...
                elif self.current_group_id == 6 and hasattr(self, 'enemy') and (
                        abs(r.logic_pos[0] - self.enemy.logic_pos[0]) + abs(
                        r.logic_pos[1] - self.enemy.logic_pos[1]) <= 1):
                    if self.sim_status in ["Đang chạy"] or self.sim_status.startswith("Chờ bước tiếp"):
                    ...
                    if self.state == "MENU" and event.button == 1:
                        for ai_name, rect in self.menu_buttons.items():
                            if rect.collidepoint(event.pos):
                                ...
                                # --- TỰ ĐỘNG CHUYỂN MAP NẾU LÀ ĐỐI KHÁNG HOẶC CSP ---
                                if self.current_group_id == 6:
                                    self.setup_adversarial_map()
                                elif self.current_group_id == 5:
                                    self.setup_csp_map()
                                else:
                            ...
                            for btn_name, rect in list(self.sim_buttons.items()):
                                if rect.collidepoint(event.pos):
                                    ...
                                    # ----------------------------
                                        ...
                                        self.edit_mode = (self.edit_mode + 1) % 4
