...
class RicochetArena:
    ...
    def run_ids(self, start, obs):
        ...
        report_lines = ["=== BÁO CÁO CHI TIẾT THUẬT TOÁN IDS ===", f"Trạng thái xuất phát: {start}", f"Mục tiêu (Goal): {self.target_pos}", ""]
    ...
    def run_sensorless(self, start, obs):
        ...
        while frontier:
            ...
            if current_1 == self.target_pos and current_2 == self.target_pos:
                ...
                try:
                    with open(self.get_map_path("BaoCao_ConformantPlanning.txt"), "w", encoding="utf-8") as f:
                        f.write("\n".join(report_lines))
    ...
    def run_partial_observable(self, start, obs):
        ...
        try:
            with open(self.get_map_path("BaoCao_PartialObservable.txt"), "w", encoding="utf-8") as f:
                f.write("\n".join(report_lines))
        except:
            pass
        return path
    ...
    def run_backtracking(self, start, obs):
        ...
        report_lines = ["=== BÁO CÁO CHI TIẾT THUẬT TOÁN BACKTRACKING (CSP) ===", f"Trạng thái xuất phát: {start}", ""]
        domains = [self.get_valid_y_domain(1, start), self.get_valid_y_domain(2, start),
                   self.get_valid_y_domain(3, start)]
        report_lines.append(f"Miền giá trị hợp lệ ban đầu của các biến (Y): {domains}")

        # [MÃ GIẢ]: function BACKTRACK(assignment, csp) returns a solution or failure
        def backtrack(assignment):
            report_lines.append(f"Đang thử gán: {assignment}")
            # Cập nhật Giao diện
            if len(assignment) > 0:
                for i, y_val in enumerate(assignment): self.aux_robots[i].visual_pos = [(i + 1) * self.cell_size,
            ...
            if len(assignment) == 3:
                if self.calculate_csp_conflicts(assignment, start) == 0:
                    report_lines.append("  => Gán đủ 3 biến, không có xung đột -> Thành công.")
                    return assignment
                report_lines.append("  => Xung đột! Quay lui (Backtrack).")
                return "failure"
            ...
            for value in domains[col_idx]:
                report_lines.append(f"  + Thử gán biến {col_idx+1} = {value}")
            ...
            report_lines.append(f"  => Hết miền giá trị cho biến {col_idx+1}, quay lui.")
            # [MÃ GIẢ]: return failure
            return "failure"
        ...
        if res != "failure":
            ...
            report_lines.append(f"\n=> [THÀNH CÔNG] Tìm thấy nghiệm Y = {res}")
            for i, aux in enumerate(self.aux_robots): aux.logic_pos = [i + 1, res[i]]; aux.start_pos = [i + 1, res[i]]
            try:
                with open(self.get_map_path("BaoCao_Backtracking.txt"), "w", encoding="utf-8") as f:
                    f.write("\n".join(report_lines))
            except: pass
            return [self.target_pos]
            
        report_lines.append("\n=> [THẤT BẠI] Không tìm thấy nghiệm.")
        try:
            with open(self.get_map_path("BaoCao_Backtracking.txt"), "w", encoding="utf-8") as f:
                f.write("\n".join(report_lines))
        except: pass
    ...
    def run_ac3(self, start, obs):
        ...
        report_lines = ["=== BÁO CÁO RÚT GỌN MIỀN GIÁ TRỊ (AC-3) ===",
                        "Mục tiêu: Đảm bảo tính nhất quán cung (Arc Consistency) trước khi chạy Backtracking để tiết kiệm thời gian.",
                        ""]

        domains = {0: self.get_valid_y_domain(1, start), 1: self.get_valid_y_domain(2, start),
                   2: self.get_valid_y_domain(3, start)}
        ...
        def rm_inconsistent_values(xi, xj):
            ...
            for x in list(domains[xi]):
                ...
                if not any(satisfy(xi, x, xj, y) for y in domains[xj]):
                    ...
                    removed = True
                    report_lines.append(
                        f"  [-] Loại bỏ giá trị Y={x} khỏi Trợ thủ {xi + 1} vì không có giá trị nào của Trợ thủ {xj + 1} khớp ràng buộc.")
        ...
        while queue:
            ...
            if rm_inconsistent_values(xi, xj):
                report_lines.append(
                    f"  => Miền giá trị của Trợ thủ {xi + 1} đã thay đổi. Cập nhật lại các cung liên quan vào hàng đợi.")
        ...
        try:
            with open(self.get_map_path("BaoCao_AC3.txt"), "w", encoding="utf-8") as f:
                f.write("\n".join(report_lines))
        ...
        res = backtrack([])
        if res != "failure":
            for i, aux in enumerate(self.aux_robots): aux.visual_pos = [(i + 1) * self.cell_size,
                                                                        res[i] * self.cell_size]; aux.logic_pos = [
                i + 1, res[i]]; aux.start_pos = [i + 1, res[i]]
            return [self.target_pos]
        return []

    def run_min_conflicts(self, start, obs):
        self.log_msg("--- MIN-CONFLICTS ---", (255, 140, 0))
        report_lines = ["=== BÁO CÁO CHI TIẾT THUẬT TOÁN MIN-CONFLICTS (CSP) ===", f"Trạng thái xuất phát: {start}", ""]
        domains = [self.get_valid_y_domain(1, start), self.get_valid_y_domain(2, start),
                   self.get_valid_y_domain(3, start)]
        ...
        report_lines.append(f"Trạng thái gán ngẫu nhiên ban đầu: {current}")

        # [MÃ GIẢ]: for i = 1 to max_steps do
        for step in range(50):
            conflicts = self.calculate_csp_conflicts(current, start)
            report_lines.append(f"\nBước {step+1}: Trạng thái gán hiện tại {current} | Số xung đột = {conflicts}")

            # Cập nhật giao diện
            for i, y_val in enumerate(current): self.aux_robots[i].visual_pos = [(i + 1) * self.cell_size,
                                                                                 y_val * self.cell_size]
            if getattr(self, 'step_mode', False):
            ...
            if conflicts == 0:
                ...
                report_lines.append(f"\n=> [THÀNH CÔNG] Tìm thấy nghiệm Y = {current} sau {step+1} bước.")
                for i, aux in enumerate(self.aux_robots): aux.logic_pos = [i + 1, current[i]]; aux.start_pos = [i + 1, current[i]]
                try:
                    with open(self.get_map_path("BaoCao_MinConflicts.txt"), "w", encoding="utf-8") as f:
                        f.write("\n".join(report_lines))
                except: pass
                return [self.target_pos]
            ...
            report_lines.append(f"  -> Chọn ngẫu nhiên biến (Trợ thủ) có thể gây xung đột: Biến {var+1}")
            ...
            for v in domains[var]:
                ...
                report_lines.append(f"    + Thử giá trị {v}: Xung đột = {c}")
                if c < min_c:
                    min_c = c; best_vals = [v]
                elif c == min_c:
                    best_vals.append(v)
            ...
            report_lines.append(f"  -> Chọn giá trị {chosen_v} cho biến {var+1} (Xung đột mới: {min_c})")

        # [MÃ GIẢ]: return failure
        report_lines.append("\n=> [THẤT BẠI] Quá 50 bước nhưng vẫn còn xung đột.")
        try:
            with open(self.get_map_path("BaoCao_MinConflicts.txt"), "w", encoding="utf-8") as f:
                f.write("\n".join(report_lines))
        except: pass
    ...
    def draw_simulation(self):
        ...
        if self.current_ai == "Sensorless":
            ...
            if r_active:
                ...
                if r_active.name == "Người Chơi": pygame.draw.circle(self.screen, (0, 0, 0), (rx, ry - 4),
