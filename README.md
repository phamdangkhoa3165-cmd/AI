...
Chương trình hỗ trợ cả Tìm kiếm mù (Blind Search), Tìm kiếm có kinh nghiệm (Heuristic Search) và Tìm kiếm cục bộ (Local Search). Quy tắc tính Heuristic được tinh chỉnh bám sát theo yêu cầu đặc thù của môn học:
...
* **Simple Hill Climbing (Leo đồi đơn giản):** * Sinh tuần tự từng trạng thái lân cận. Sẽ nhảy ngay lập tức sang lân cận đầu tiên sinh ra có chi phí Heuristic $h(n)$ nhỏ hơn trạng thái hiện tại.
* **Steepest-Ascent Hill Climbing (Leo đồi dốc nhất):** * Khảo sát TOÀN BỘ lân cận và chọn lân cận có chi phí $h(n)$ thấp nhất. Nếu tốt hơn trạng thái hiện tại thì mới tiến hành nhảy bước.
  * *Lưu ý:* Hàm đánh giá được sử dụng là **khoảng cách Manhattan**. Cả 2 thuật toán Leo đồi đều có thể dừng sớm nếu rơi vào trạng thái cực tiểu cục bộ (Local Minimum).
...
2. Mở file `UI_8_puzzle.ipynb` hoặc `UI_8_puzzle.py` bằng Jupyter Notebook, JupyterLab hoặc VS Code và chạy trình biên dịch.